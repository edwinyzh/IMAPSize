...
const
    kGifErrorMessages:      array[0..25] of string = (
        'no error',                                                 // 0
...
while (not done) do
    ...
    else fIOStream.Read(b, 1);                  // image separator
...
// a comment extension
        ...
        except  // Allow end of file in comment if at least one image  {LDB}
...
{This is a single bitmap containing all the frames.  A mask is also provided
 if the GIF is transparent.  Each Frame is setup so that it can be transparently
...
try
  ...
  for i := 0 to (fImageDescriptorList.Count - 1) do  {for all the frames}
    ...
    if (i < fImageDescriptorList.Count-1) then
      ...
      if IsTransparent and (ImageDisposal[i] = 2) then
        with id^ do
          for Y := Height-1 downto Height-rHeight do
            begin
